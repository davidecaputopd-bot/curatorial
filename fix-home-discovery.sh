...
cp src/app/page.tsx src/app/page.backup-before-discovery-fix.tsx
cp src/app/api/feed/route.ts src/app/api/feed.route.backup-before-discovery-fix.ts

echo "→ Riscrivo API feed: solo immagini, piattaforme curate, niente RSS/notizie"
cat > src/app/api/feed/route.ts <<'EOF'
...
echo "→ Patch Home: niente categorie visibili, niente category filter"
python3 - <<'PY'
...
# Home deve chiamare il feed immagini senza categoria.
s = re.sub(
  r"""const fetchPage = \(cat: string \| null, offset: number\) => \{\s*const p = cat \? '&category=' \+ cat : ''\s*return fetch\(`/api/feed\?type=image&limit=\$\{PAGE_SIZE\}&offset=\$\{offset\}&seed=\$\{feedSeed\.current\}\$\{p\}`\)\.then\(\(r\) => r\.json\(\)\)\s*\}""",
...
s = s.replace(
  "return fetch(`/api/feed?limit=${PAGE_SIZE}&offset=${offset}&seed=${feedSeed.current}`).then((r) => r.json())",
  "return fetch(`/api/feed?type=image&limit=${PAGE_SIZE}&offset=${offset}&seed=${feedSeed.current}`).then((r) => r.json())"
)

# Disattiva categoria attiva nella Home.
s = s.replace("setActive(cat)", "setActive(null)")
s = s.replace("const d = await fetchPage(cat, 0)", "const d = await fetchPage(null, 0)")
s = s.replace("const d = await fetchPage(active, offset)", "const d = await fetchPage(null, offset)")

# Togli qualunque rendering basato su categoryLabels in Home.
s = s.replace("Object.entries(categoryLabels).map", "Object.entries({}).map")
s = s.replace("Object.keys(categoryLabels).map", "Object.keys({}).map")
s = s.replace("categoryLabels).map", "{}).map")

# Nelle card non mostrare categoria: mostra piattaforma.
s = s.replace(
...
s = s.replace("opacity-0 group-hover:opacity-100", "opacity-100")
s = s.replace("opacity-0 transition-opacity group-hover:opacity-100", "opacity-100 transition-opacity")
s = s.replace("group-hover:opacity-100", "opacity-100")

# Copy più corretto.
s = s.replace(
  "Nessuna immagine disponibile. Popola GROW con Are.na o fetch-images.",
  "Nessuna reference disponibile. Aggiorna Are.na e piattaforme immagini."
)

# Elimina testi categoria evidenti se presenti.
s = s.replace("Categorie", "Archivio")
s = s.replace("Scegli una categoria", "Scopri reference visive")

p.write_text(s)
PY

echo "→ Archivio: qui invece lasciamo categorie/filtri"
python3 - <<'PY'
from pathlib import Path

p = Path("src/app/archivio/page.tsx")
if p.exists():
    s = p.read_text()
    s = s.replace("opacity-0 group-hover:opacity-100", "opacity-100")
    s = s.replace("opacity-0 transition-opacity group-hover:opacity-100", "opacity-100 transition-opacity")
...
echo "→ Pulizia file temporanei pericolosi/inutili"
rm -f set-grow-pin.mjs
rm -f grow-image-feed-patch.sh
rm -f src/app/login/page.email-backup.tsx

echo "→ Controllo che Home usi solo type=image"
grep -n "api/feed" src/app/page.tsx || true

echo "→ Build"
npm run build

echo "→ Commit e deploy"
git add src/app/page.tsx src/app/archivio/page.tsx src/app/api/feed/route.ts
git commit -m "Fix discovery home feed quality and remove categories" || echo "Nessuna modifica da committare"
git push origin main
npx vercel --prod

echo "✓ Deploy finito."
echo "Ora apri:"
echo "https://grow-eight-kappa.vercel.app/api/fetch-arena"
echo "https://grow-eight-kappa.vercel.app/api/fetch-images"
echo "Poi hard refresh Home: CMD + SHIFT + R"
